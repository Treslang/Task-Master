...
public class ViewTasks extends BaseAdapter {
    private Activity activity;
    private ArrayList<HashMap<String, String>> data;
    private DBHelper2 database2;

    public ViewTasks(Activity a, ArrayList<HashMap<String, String>> d, DBHelper2 mydb2) {
        activity = a;
        data = d;
        database2 = mydb2;
    }
        ...
        try {
            ...
            holder.checkBtn.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                ...
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    if (isChecked) {
                        database2.updateTaskStatus(singleTask.get("id"), 1);
                        tmpHolder.task_name.setText(Html.fromHtml("<strike>" + singleTask.get("task") + "</strike>"));
                    } else {
                        database2.updateTaskStatus(singleTask.get("id"), 0);
